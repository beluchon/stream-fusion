...
class Torbox(BaseDebrid):
    ...
    async def add_magnet(self, magnet, ip=None, privacy="private"):
        ...
        try:
            response = await self.json_response(url, method='post', headers=await self.get_headers(), data=data)
            logger.info(f"Torbox: Réponse d'ajout de magnet: {response}")
            return response
        except Exception as e:
            logger.error(f"Torbox: Erreur lors de l'ajout du magnet: {str(e)}")
            return None
    ...
    async def add_torrent(self, torrent_file, privacy="private"):
        ...
        try:
            response = await self.json_response(url, method='post', headers=await self.get_headers(), data=data, files=files)
            logger.info(f"Torbox: Réponse d'ajout de fichier torrent: {response}")
            return response
        except Exception as e:
            logger.error(f"Torbox: Erreur lors de l'ajout du fichier torrent: {str(e)}")
            return None
    ...
    async def get_torrent_info(self, torrent_id):
        ...
        try:
            response = await self.json_response(url, headers=await self.get_headers())
            logger.debug(f"Torbox: Réponse d'info torrent: {response}")
            return response
        except Exception as e:
            logger.error(f"Torbox: Erreur lors de la récupération des informations du torrent: {str(e)}")
            return None
    ...
    async def control_torrent(self, torrent_id, operation):
        ...
        try:
            response = await self.json_response(url, method='post', headers=await self.get_headers(), data=data)
            logger.info(f"Torbox: Réponse de contrôle du torrent: {response}")
            return response
        except Exception as e:
            logger.error(f"Torbox: Erreur lors du contrôle du torrent: {str(e)}")
            return None
    ...
    async def request_download_link(self, torrent_id, file_id=None, zip_link=False):
        logger.info(f"Torbox: Demande de lien de téléchargement pour le torrent ID: {torrent_id}, file ID: {file_id}, zip link: {zip_link}")
    ...
    async def get_stream_link(self, query, config, ip=None):
        logger.info("Torbox: Récupération du lien de streaming")
        magnet = query['magnet']
        ...
        if existing_torrent:
            ...
            torrent_info = await self.add_magnet_or_torrent(magnet, torrent_download)
    ...
    async def get_availability_bulk(self, hashes_or_magnets, ip=None):
        logger.info(f"Torbox: Vérification de la disponibilité pour {len(hashes_or_magnets)} éléments")
    ...
    async def _find_existing_torrent(self, info_hash):
        logger.info(f"Torbox: Recherche d'un torrent existant avec le hash: {info_hash}")
        
        try:
            torrents = await self.json_response(f"{self.base_url}/torrents/mylist", headers=await self.get_headers())
            if torrents and "data" in torrents:
                for torrent in torrents["data"]:
                    if torrent["hash"].lower() == info_hash.lower():
                        logger.info(f"Torbox: Torrent existant trouvé avec l'ID: {torrent['id']}")
                        return torrent
            logger.info("Torbox: Aucun torrent existant trouvé")
            return None
        except Exception as e:
            logger.error(f"Torbox: Erreur lors de la recherche d'un torrent existant: {str(e)}")
            return None

    async def add_magnet_or_torrent(self, magnet, torrent_download=None, ip=None, privacy="private"):
        try:
            if torrent_download is None:
                logger.info("Torbox: Ajout du magnet")
                response = await self.add_magnet(magnet, ip, privacy)
            else:
                logger.info("Torbox: Téléchargement et ajout du fichier torrent")
                torrent_file = await self.download_torrent_file(torrent_download)
                response = await self.add_torrent(torrent_file, privacy)

            logger.info(f"Torbox: Réponse d'ajout de torrent: {response}")

            if not response or "data" not in response or response["data"] is None:
                logger.error("Torbox: Échec de l'ajout du magnet/torrent")
                return None

            return response["data"]
        except Exception as e:
            logger.error(f"Torbox: Erreur lors de l'ajout du magnet/torrent: {str(e)}")
            return None

    async def _wait_for_torrent_completion(self, torrent_id, timeout=60, interval=10):
        logger.info(f"Torbox: Attente de la fin du téléchargement du torrent, ID: {torrent_id}")
        ...
        async def check_status():
            ...
            if torrent_info and "data" in torrent_info:
                ...
                logger.info(f"Torbox: Statut actuel du torrent: {torrent_info['data']['download_state']}")
                return True if len(files) > 0 else False
            return False

        result = await self.wait_for_ready_status(check_status, timeout, interval)
        if result:
            logger.info("Torbox: Le torrent est prêt")
        else:
            logger.warning("Torbox: Délai d'attente dépassé pour le torrent")
        return result

    async def _select_file(self, torrent_info, stream_type, file_index, season, episode):
        logger.info(f"Torbox: Sélection du fichier pour {stream_type}, file_index: {file_index}, saison: {season}, épisode: {episode}")
